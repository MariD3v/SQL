...
SELECT nombre_cliente, empleado.nombre FROM cliente, empleado, pago WHERE pago.codigo_cliente = cliente.codigo_cliente AND codigo_empleado_rep_ventas = codigo_empleado;       
/*3. Muestra el nombre de los clientes que no hayan realizado pagos junto con el nombre de sus representantes de ventas.*/
SELECT nombre_cliente, empleado.nombre FROM cliente LEFT JOIN pago ON pago.codigo_cliente = cliente.codigo_cliente INNER JOIN  empleado ON codigo_empleado_rep_ventas = codigo_empleado WHERE pago.codigo_cliente IS NULL;  
/*4. Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.*/
SELECT cliente.nombre_cliente, empleado.nombre, oficina.ciudad  FROM cliente INNER JOIN pago ON cliente.codigo_cliente=pago.codigo_cliente INNER JOIN empleado ON cliente.codigo_empleado_rep_ventas=empleado.codigo_empleado INNER JOIN oficina ON empleado.codigo_oficina=oficina.codigo_oficina;
/*5. Devuelve el nombre de los clientes que no hayan hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.*/
SELECT nombre_cliente, empleado.nombre, oficina.ciudad FROM cliente LEFT JOIN pago ON pago.codigo_cliente = cliente.codigo_cliente INNER JOIN  empleado ON codigo_empleado_rep_ventas = codigo_empleado INNER JOIN oficina ON oficina.codigo_oficina = empleado.codigo_oficina WHERE pago.codigo_cliente IS NULL;  
/*6. Lista la dirección de las oficinas que tengan clientes en Fuenlabrada.*/
SELECT oficina.linea_direccion1 FROM oficina, empleado, cliente WHERE codigo_empleado_rep_ventas = codigo_empleado AND empleado.codigo_oficina = oficina.codigo_oficina AND cliente.ciudad = "Fuenlabrada"; 
/*7. Devuelve el nombre de los clientes y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.*/
SELECT nombre_cliente, empleado.nombre, oficina.ciudad FROM oficina, empleado, cliente WHERE cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado AND empleado.codigo_oficina = oficina.codigo_oficina;       
/*8. Devuelve un listado con el nombre de los empleados junto con el nombre de sus jefes.*/
SELECT A.nombre AS empleado, B.nombre AS jefe FROM empleado A, empleado B WHERE A.codigo_empleado = B.codigo_jefe;
SELECT A.nombre AS empleado, B.nombre AS jefe FROM empleado A INNER JOIN empleado B ON A.codigo_empleado = B.codigo_jefe;       
/*9. Devuelve un listado que muestre el nombre de cada empleados, el nombre de su jefe y el nombre del jefe de sus jefe.*/
SELECT A.nombre AS empleado, B.nombre AS jefe, C.nombre AS "Jefe de jefe" FROM empleado A, empleado B, empleado C WHERE A.codigo_empleado = B.codigo_jefe AND B.codigo_empleado = C.codigo_jefe;       
/*10. Devuelve el nombre de los clientes a los que no se les ha entregado a tiempo un pedido.*/
SELECT nombre_cliente FROM cliente INNER JOIN pedido ON cliente.codigo_cliente = pedido.codigo_cliente WHERE fecha_esperada < fecha_entrega;
...
SELECT COUNT(*) , ciudad FROM cliente WHERE ciudad LIKE "M%" GROUP BY ciudad;
/*9.Devuelve el nombre de los representantes de ventas y el número de clientes al que atiende cada uno.*/
SELECT empleado.nombre, COUNT(*) FROM empleado, cliente WHERE codigo_empleado_rep_ventas = codigo_empleado GROUP BY empleado.codigo_empleado, empleado.nombre;
/*10.Calcula el número de clientes que no tiene asignado representante de ventas.*/
SELECT COUNT(codigo_cliente) FROM cliente LEFT JOIN empleado ON codigo_empleado_rep_ventas = codigo_empleado WHERE codigo_empleado_rep_ventas IS NULL;
